...
class SwinTransformerFUS(BaseModule):
    """ Swin Transformer
    ...
    def __init__(self,
        ...
        super(SwinTransformerFUS, self).__init__(init_cfg=init_cfg)
        ...
        if self.Fusion == 'MFB' or 'BGF':
            pds=0
            KernelS =1
            num_filters=3
            self.bev_fc1 = nn.Sequential(
                nn.Conv2d(3, num_filters, KernelS, stride=1, padding=pds, bias=False),
    ...
    def trad_fusion(self, RGB, LIDAR):
        cyl_x1 = self.cyl_fc1(RGB)
        bev_x1 = self.bev_fc1(LIDAR)
        ...
        conv_features = self.fusion_transform(point_features)
        #conv_features = conv_features.squeeze(0).transpose(0, 1)
        return conv_features
    def MFB(self, RGB, LIDAR):
        ...
        combined = self.combine_trans(inter)

        point_features = torch.cat([combined, feat_aux], dim=1)
        out = self.fusion_transform_MFB(point_features)

        out = torch.sqrt(self.out_relu(out)) + torch.sqrt(self.out_relu(-out))
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformerFUS, self).train(mode)
        self._freeze_stages()
    ...
    def forward(self, x):
        ...
        if self.Fusion == 'RGB':
            ...
            x = self.MFBv2(x[:,:3],x[:,3:])                 
        elif self.Fusion == 'BGF':
            x = self.trad_fusion(x[:,:3],x[:,3:])
        elif self.Fusion == 'Focal':
            x =x.transpose(1, 3)
            x = self.Focal(x)
            x =x.transpose(3, 1) 
            x=self.focalconv(x)
        elif self.Fusion == 'FFM':
            x = self.FFM(x1,x2)
